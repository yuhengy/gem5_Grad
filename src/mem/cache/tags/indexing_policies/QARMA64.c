...
typedef unsigned long long int QARMA64_key_t;
...
text_t pseudo_reflect(text_t is, QARMA64_key_t tk) {
	cell_t cell[16];
	text2cell(cell, is);
...
text_t forward(text_t is, QARMA64_key_t tk, int r) {
...
text_t backward(text_t is, QARMA64_key_t tk, int r) {
	cell_t cell[16];
	text2cell(cell, is);
	//printf("0x%016llx\n", is);
...
QARMA64_key_t forward_update_key(QARMA64_key_t T) {
...
QARMA64_key_t backward_update_key(QARMA64_key_t T) {
	cell_t cell[16], temp[16];
	text2cell(cell, T);
...
text_t qarma64_enc(text_t plaintext, tweak_t tweak, QARMA64_key_t w0, QARMA64_key_t k0, int rounds) {
	QARMA64_key_t w1 = ((w0 >> 1) | (w0 << (64 - 1))) ^ (w0 >> (16 * m - 1));
	QARMA64_key_t k1 = k0;
...
text_t qarma64_dec(text_t plaintext, tweak_t tweak, QARMA64_key_t w0, QARMA64_key_t k0, int rounds) {
	...
	QARMA64_key_t k1 = cell2text(k1_cell);
...
int main() {
	...
	QARMA64_key_t w0 = 0x84be85ce9804e94b;
	QARMA64_key_t k0 = 0xec2802d4e0a488e9;
